...
def open_or_download_file(file_path):
    ...
    if os.path.exists(absolute_file_path):
        ...
        if mime_type and mime_type.startswith('text'):
            ...
            # Extract text from DOCX and render as HTML
            ...
            # Extract text from PPTX and render as HTML
            text = extract_text_from_pptx(absolute_file_path)
            return render_template_string('<pre>{{ content }}</pre>', content=text)
        else:
            # Send the file for in-browser viewing
            return send_file(absolute_file_path, mimetype=mime_type, as_attachment=False)
    else:
